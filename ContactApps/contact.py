...
class Contact:
	...
	def update_contact_mail(self, name):
		print(f'Old Mail \t:{self.contact_list[name]["mail"]}')
		new_mail = input(f'New Mail \t: ')
		confirm = input('Are you sure to update ? (y/n) ')
		if confirm.lower() == 'y':
			self.contact_list[name]['mail'] = new_mail
			self.config.save_data(self.contact_list, self.config.data_json_path)
			print('Contact Updated.')
		else:
			print('Canceled.')
	...
	def update_contact(self):
		...
		if res:
			...
			if option == '1':
				...
				self.update_contact_mail(search_contact)
			else:
				pass
